...
def list_files(request):
    ...
    allowFiles = {
        "listfile": USettings.UEditorUploadSettings.get,
        "listimage": USettings.UEditorUploadSettings.get
    }
    listSize = {
        "listfile": USettings.UEditorUploadSettings.get,
        "listimage": USettings.UEditorUploadSettings.get
    ...
    # 取得参数
    list_size = int(request.GET.get)
    list_start = int(request.GET.get)
...
def get_output_path(request, path_format, path_format_var):
    # 取得输出文件的路径
    OutputPathFormat = (request.GET.get % path_format_var).replace("\\", "/")
    # 分解OutputPathFormat
